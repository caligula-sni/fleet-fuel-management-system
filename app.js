/* ═══════════════════════════════════════════════════════
   ...
   Units  : KM, Liters, Philippine Peso (₱)
   Labels : English only
   Data   : Filipino driver names, Pampanga locations
═══════════════════════════════════════════════════════ */
'use strict';
...
async function loadAllData() {
  try {
    ...
    // Mark loaded so navigation still works — error shown in content area only
    STATE.loaded = true;
    showServerError();
  }
}
...
function showServerError() {
  // Replace only the home page content — nav stays fully working
...
// RECORDS
// Calculations:
//   distance (km)  = endOdometer - startOdometer
//   km per liter   = distance / liters
//   cost per km    = cost / distance
// ══════════════════════════════════════════════════════
...
function initDashboard() {
  ...
  const drivers = [];
  STATE.vehicles.forEach(function(v) {
    if (plates.indexOf(v.plate) === -1)    plates.push(v.plate);
    if (drivers.indexOf(v.driver) === -1)  drivers.push(v.driver);
  });
  plates.sort(); drivers.sort();
