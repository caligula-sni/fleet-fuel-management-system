  ...
  if (homeEl) {
    homeEl.innerHTML = `
      <div style="display:flex;flex-direction:column;align-items:center;
                  ...
                  font-family:Segoe UI,Arial;color:#1B5E20;
                  text-align:center;padding:32px">
        <div style="font-size:48px">⚠️</div>
        <div style="font-size:22px;font-weight:700">Server Not Running</div>
...
async function saveVehicle() {
  ...
  const entry = {
    plate: plate, name: name, driver: driver,
  ...
  if (editingVehicleIdx >= 0) {
    ...
    if (STATE.vehicles.find(function(v) { return v.plate === plate; })) {
...
function renderConfigList(elId, items, key) {
  ...
  items.forEach(function(item, i) {
    ...
    li.innerHTML =
      '<span>' + item + '</span>' +
      '<button class="action-btn del" onclick="deleteConfigItem(\'' + key + '\',' + i + ')">✕</button>';
    ul.appendChild(li);
  });
}
...
function renderRecords() {
  ...
  STATE.records.forEach(function(r, i) {
    ...
    tr.innerHTML =
      ...
      '<td>' + (r.docNum  || '—') + '</td>' +
      '<td>' + (r.comment || '—') + '</td>' +
      '<td><button class="action-btn del" onclick="deleteRecord(event,' + i + ')">DELETE</button></td>';
    tr.style.cursor = 'pointer';
    tr.title = 'Click to edit';
    tr.onclick = function() { openRecordForm('edit', i); };
    tbody.appendChild(tr);
  });
}

async function deleteRecord(event, i) {
...
function openRecordForm(mode, idx) {
  ...
  ['fEndMile','fGallon','fCost','fDistGal','fCostMile'].forEach(function(id) {
  ...
  if (mode === 'new') {
    ...
    ['fPlate','fStart','fEnd','fDocType','fDocNum','fComment'].forEach(function(id) {
      document.getElementById(id).value = '';
    });
    document.getElementById('fVehicle').value    = '';
    document.getElementById('fDriver').innerHTML = '<option value="">— Select —</option>';
    ['fStartMile','fEndMile','fDistance','fGallon','fCost','fDistGal','fCostMile'].forEach(function(id) {
      document.getElementById(id).value = '';
    });
...
function calcFields() {
  ...
  check('fEndMile',
    thr.tripAlert > 0 && dist > 0 && dist > thr.tripAlert,
    'Distance ' + dist + ' km exceeds trip alert of ' + thr.tripAlert + ' km');

  check('fEndMile',
    endOdo > 0 && startOdo > 0 && dist < 0,
  ...
  check('fDistGal',
    thr.alertKmL > 0 && liters > 0 && dist > 0 && kmL < thr.alertKmL,
...
function setReportMode(mode) {
  ...
  if (mode === 'driver') {
    Array.from(driverSet).sort().forEach(function(d) {
      const o = document.createElement('option');
      o.value = d; o.textContent = d; sel.appendChild(o);
    });
  } else {
    Array.from(plateSet).sort().forEach(function(p) {
      const o = document.createElement('option');
